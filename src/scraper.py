...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    try:
        # ⚡ Bolt: Use provided session to pool connections, otherwise fallback to requests.post
        if session:
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Pool connections to avoid repeated TLS handshakes when sending multiple messages
