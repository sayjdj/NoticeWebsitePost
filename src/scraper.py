...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    try:
        # ⚡ Bolt: Allow optional session usage to pool connections for consecutive Telegram API requests
        requester = session if session else requests
        response = requester.post(url, json=payload, timeout=10)
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Use requests.Session() to pool connections and avoid repeated TCP/TLS handshakes
