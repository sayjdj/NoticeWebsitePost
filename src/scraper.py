...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    try:
        # Use provided session for connection pooling, otherwise fallback to requests
        poster = session if session else requests
        response = poster.post(url, json=payload, timeout=10)
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Use a single requests.Session for Telegram notifications
        # This reuses the underlying TCP connection and avoids repeated TLS handshakes
