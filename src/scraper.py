...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Use connection pooling to reduce TCP/TLS handshake overhead for consecutive notifications
        with requests.Session() as session:
            for post in reversed(new_posts):
                message = f"<b>[{html.escape(post['site_name'])}]</b>\n{html.escape(post['title'])}\n<a href='{post['link']}'>링크 이동</a>"
                send_telegram_message(telegram_token, telegram_chat_id, message, session)
