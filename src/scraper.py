...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    try:
        # ⚡ Bolt: Use connection pooling if session is provided
        post_func = session.post if session else requests.post
        response = post_func(url, json=payload, timeout=10)
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Use requests.Session for connection pooling to avoid repeated TLS handshakes
