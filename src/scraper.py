...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    try:
        # ⚡ Bolt: Use connection pooling if session provided to avoid repeated TLS handshakes
        req_func = session.post if session else requests.post
        response = req_func(url, json=payload, timeout=10)
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Initialize requests.Session for connection pooling across consecutive notifications
