...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    try:
        # ⚡ Bolt: Use provided session to reuse TCP/TLS connection
        if session:
        ...
        logger.error(f"Failed to send Telegram message due to network error: {e}")
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Use a session to pool connections for multiple Telegram notifications
