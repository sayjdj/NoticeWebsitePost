...
def send_telegram_message(token, chat_id, message, session=None):
    ...
    request_method = session.post if session else requests.post
...
def main():
    ...
    if new_posts:
        ...
        # ⚡ Bolt: Use a single requests.Session for connection pooling when sending multiple notifications
