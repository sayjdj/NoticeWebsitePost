...
## 2024-05-18 - Concurrent Scraping I/O Optimization
**Learning:** Sequential scraping of target sites leads to an O(N) wait time due to individual network request delays. The scraper script inherently performs independent I/O tasks per site configuration.
**Action:** Always use concurrency models like `concurrent.futures.ThreadPoolExecutor` when performing multiple independent network requests to decrease total execution time significantly without sacrificing readability.
