...
## 2024-05-24 - Telegram Notification Loop Connection Pooling
**Learning:** Sequential, repeated API requests to the same host (e.g., api.telegram.org) without connection pooling incur significant overhead due to repeated TCP/TLS handshakes.
**Action:** Always wrap consecutive, synchronous network requests to the same endpoint in a `requests.Session()` context manager to reuse connections and reduce latency.
