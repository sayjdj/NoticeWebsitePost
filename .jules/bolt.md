...
## 2024-05-23 - Connection Pooling for Multiple Notifications
**Learning:** When finding multiple new posts, the scraper sequentially calls the Telegram API. Using `requests.post()` creates a new TCP/TLS connection for each request, adding significant overhead.
**Action:** When making consecutive API requests to the same host (like sending notifications), use `requests.Session()` to enable connection pooling and avoid repeated handshakes.
