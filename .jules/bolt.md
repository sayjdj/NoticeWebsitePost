...
## 2024-06-15 - Connection Pooling for Telegram Notifications
**Learning:** Reusing connections using `requests.Session()` avoids the overhead of repeated TCP connections and TLS handshakes when making sequential API requests to the same host in Python.
**Action:** Use connection pooling for notifications and similar sequential network requests to reduce unnecessary I/O wait times.
