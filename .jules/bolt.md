...
## 2024-05-24 - Connection Pooling for Consecutive API Requests
**Learning:** Sending consecutive API requests (like Telegram notifications) to the same host using `requests.post` inside a loop creates a new connection for each request. This incurs significant overhead from repeated TCP and TLS handshakes, slowing down execution.
**Action:** Always use `requests.Session()` to pool connections when making multiple consecutive requests to the same host to reuse the underlying TCP/TLS connection.
