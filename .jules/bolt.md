...
## 2026-06-22 - Connection Pooling for Telegram Notifications
**Learning:** Sending consecutive notifications to the Telegram API using `requests.post()` directly within a loop incurs significant overhead due to repeated TCP connection setups and TLS handshakes for every request.
**Action:** When making multiple consecutive API requests to the same host (such as sending batch Telegram notifications), wrap the requests in a `requests.Session()` context manager to pool connections and dramatically improve execution speed.
