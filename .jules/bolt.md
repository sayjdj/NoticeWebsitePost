...
## 2024-05-18 - Telegram Notifications Connection Pooling
**Learning:** Burst Telegram notifications without connection pooling cause a performance bottleneck due to repeated TCP/TLS handshakes in this cron-based architecture.
**Action:** Use `requests.Session()` as a context manager when sequentially sending multiple requests to the same host to pool connections and reduce overhead.
