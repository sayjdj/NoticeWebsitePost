...
## 2024-05-28 - Connection Pooling for Telegram Notifications
**Learning:** Sending consecutive API requests (like Telegram notifications) using `requests.post()` creates a new TCP/TLS connection for each request, which adds significant overhead and delay, particularly when notifying about many new posts.
**Action:** When making multiple requests to the same host in succession, always use `requests.Session()` to enable connection pooling, which keeps the underlying TCP connection open and dramatically reduces the overhead of subsequent requests.
