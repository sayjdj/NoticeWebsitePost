...
## 2024-05-24 - Telegram Notification Connection Pooling
**Learning:** Sending multiple consecutive Telegram API notifications using `requests.post()` in a loop creates a new TCP connection and TLS handshake for every message, which is extremely inefficient and slow when broadcasting to many users or sending multiple updates.
**Action:** Always use `requests.Session()` to pool connections when sending consecutive API requests to the same host (e.g., Telegram). Pass the session object to the notification sender function.
