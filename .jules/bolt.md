...
## 2024-05-19 - Connection Pooling for Consecutive API Requests
**Learning:** Sending consecutive HTTP requests to the same host (e.g., Telegram notifications in `src/scraper.py`) using `requests.post()` creates a new TCP connection and performs a new TLS handshake for each request, adding significant overhead.
**Action:** When performing multiple requests to the same API host in a sequence or loop, use `requests.Session()` to pool connections and reuse the underlying TCP/TLS socket, significantly reducing network latency and overhead.
