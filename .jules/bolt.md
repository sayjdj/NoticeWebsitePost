...
## 2024-11-20 - Connection Pooling for Repeated API Calls
**Learning:** In a serverless architecture (GitHub Actions cron job), sending consecutive notifications (e.g., via the Telegram Bot API) without connection pooling results in a performance bottleneck due to the overhead of repeated TCP connections and TLS handshakes.
**Action:** When making multiple consecutive HTTP requests to the same host, always use `requests.Session()` to pool connections and reuse the underlying TCP connection and TLS session, which speeds up processing time.
