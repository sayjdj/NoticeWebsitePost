...
## 2024-05-18 - Connection Pooling for Repeated API Calls
**Learning:** The previous implementation opened a new TCP/TLS connection for every single Telegram notification sent in a loop, causing significant overhead (handshakes, latency) especially when multiple new posts were found concurrently. Using `requests.Session` pools connections dynamically and avoids this redundant overhead to the same host (`api.telegram.org`).
**Action:** When making consecutive HTTP requests to the same host (e.g., notification loops, paginated scraping), always wrap the calls in a `with requests.Session() as session:` block and pass the session to the requester functions.
