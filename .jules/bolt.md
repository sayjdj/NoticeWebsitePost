...
## 2024-05-18 - [Optimize Telegram notifications connection pooling]
**Learning:** When sending consecutive API requests to the same host (e.g., Telegram notifications in `src/scraper.py`), a new TCP connection and TLS handshake occurs for every single request when using standard `requests.post`.
**Action:** Always wrap consecutive API requests to the same domain in a `requests.Session()` context and pass the `session` object to the request method (or use `session.post(...)`) to pool connections and avoid overhead.
