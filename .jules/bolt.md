...
## 2026-06-06 - Use connection pooling for consecutive Telegram notifications
**Learning:** In a serverless script, running a loop that sends multiple outbound HTTP requests (like Telegram notifications) using `requests.post()` creates significant overhead because each call establishes a new TCP and TLS handshake.
**Action:** Use `requests.Session()` as a context manager and pass it to functions handling consecutive API requests to the same host to pool connections and reduce wait time.
