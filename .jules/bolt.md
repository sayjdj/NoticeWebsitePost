...
## 2024-11-10 - Requests Session Connection Pooling
**Learning:** Making consecutive network requests to the same host using `requests.get` or `requests.post` incurs a high overhead due to repeated TCP connection setups and TLS handshakes for each request. In a scraping script that sends multiple Telegram notifications in a loop, this can be a significant bottleneck.
**Action:** When making multiple requests to the same host (e.g., in a loop), use `requests.Session()` to pool connections and reuse the underlying TCP/TLS connection, significantly speeding up the operations.
