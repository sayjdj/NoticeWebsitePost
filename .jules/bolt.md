...
## 2024-11-09 - Connection Pooling for Repeated API Requests
**Learning:** Sending consecutive API requests (e.g., Telegram notifications in a loop) using `requests.post()` creates a new TCP connection and performs a new TLS handshake for each request, leading to significant overhead and increased latency when processing multiple items at once.
**Action:** Always use `requests.Session()` to pool connections and reuse the underlying TCP/TLS socket when sending multiple requests to the same host in succession.
