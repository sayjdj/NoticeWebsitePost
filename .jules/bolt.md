...
**Learning:** Sending consecutive notifications to the same host (like Telegram API) in a loop using `requests.post()` creates significant overhead because each request performs a new TCP/TLS handshake.
**Action:** Always use `requests.Session()` to pool connections when sending consecutive API requests to the same host, especially in a loop.
