...
## 2024-06-27 - Backend Performance: HTTP Connection Pooling
**Learning:** Sending consecutive API requests (like Telegram notifications) in a loop using `requests.post()` creates significant overhead because each call requires a new TCP connection and TLS handshake.
**Action:** When making multiple requests to the same host in Python, always wrap the calls in a `requests.Session()` block and pass the session object. This pools connections and dramatically improves execution speed for bulk operations.
