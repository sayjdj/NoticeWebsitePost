...
## 2024-05-19 - Connection pooling for Telegram notifications
**Learning:** Sequential HTTP requests to the same host (e.g., Telegram API) using `requests.post()` incur the overhead of setting up a new TCP connection and performing a TLS handshake for every single request.
**Action:** When making multiple sequential API calls to the same endpoint in the same function block, always use `requests.Session()` to pool connections and reuse the underlying TCP/TLS connection.
