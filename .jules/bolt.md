...
## 2024-11-10 - Connection Pooling for Telegram API
**Learning:** Sending multiple Telegram messages in a loop using `requests.post()` creates a new TCP/TLS connection for each request, adding significant network overhead.
**Action:** Always use `requests.Session()` to pool connections when making multiple sequential API requests to the same host to eliminate redundant handshake latency.
