...
## 2026-05-18 - Connection Pooling for Consecutive API Calls
**Learning:** In `src/scraper.py`, when notifying about newly scraped posts, the application sent multiple consecutive Telegram API requests. Using `requests.post()` repeatedly initiates a new TCP connection and TLS handshake for every single notification, which causes significant overhead and blocking.
**Action:** When sending consecutive API requests to the same host (e.g., Telegram notifications), always use `requests.Session()` to pool connections and avoid the overhead of repeated TCP/TLS handshakes.
